...
describe('Markdown task repository',()=>{
  ...
  it('persists a child task as its own Markdown file',async()=>{const{root,repository}=await repo();const parent=await repository.create('父任务');const child=await repository.create('子任务','inbox',parent.id);expect(child.parentId).toBe(parent.id);expect(parseTask(await readFile(join(root,'tasks',`${child.id}.md`),'utf8')).title).toBe('子任务')})
