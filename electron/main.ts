...
function createWindow() {
  ...
  if (process.argv.includes('--smoke')) win.webContents.once('did-finish-load', async () => {
    try {
      const result=await win.webContents.executeJavaScript(`(async()=>{const api=window.bearTodo;if(!api)throw new Error('bridge missing');const health=await api.health();const group=await api.createGroup('冒烟分组');const task=await api.createTask('冒烟任务','inbox');const [groups,tasks]=await Promise.all([api.listGroups(),api.listTasks()]);return{health,groupPersisted:groups.some(x=>x.id===group.id),taskPersisted:tasks.some(x=>x.id===task.id)}})()`)
      ...
      app.exit(result.groupPersisted&&result.taskPersisted?0:1)
