...
export type GlobalTheme='mist'|'sage'|'ocean'|'rose'|'graphite'|'forest'|'coast'|'custom'
export interface CustomTheme {accent:string;scene:string;backgroundImage:string|null}
export interface LocalSettings {schema:'bearai.todo/settings@1';workspacePath:string|null;theme:GlobalTheme;sidebarWidth:number;customTheme:CustomTheme;updatedAt:string}
...
export class SettingsRepository {
  ...
  async read():Promise<LocalSettings>{const defaults={schema:'bearai.todo/settings@1' as const,workspacePath:null,theme:'mist' as GlobalTheme,sidebarWidth:262,customTheme:{accent:'#4fa879',scene:'#e9f1ef',backgroundImage:null},updatedAt:new Date().toISOString()};try{const value=JSON.parse(await readFile(this.path,'utf8'));return{...defaults,...value,sidebarWidth:Math.min(380,Math.max(220,Number(value.sidebarWidth)||262)),customTheme:{...defaults.customTheme,...value.customTheme}}}catch{return defaults}}
  ...
  async setPreferences(patch:Partial<Pick<LocalSettings,'theme'|'sidebarWidth'|'customTheme'>>){const current=await this.read();return this.save({...current,...patch,sidebarWidth:patch.sidebarWidth===undefined?current.sidebarWidth:Math.min(380,Math.max(220,patch.sidebarWidth)),customTheme:patch.customTheme?{...current.customTheme,...patch.customTheme}:current.customTheme,updatedAt:new Date().toISOString()})}
}
